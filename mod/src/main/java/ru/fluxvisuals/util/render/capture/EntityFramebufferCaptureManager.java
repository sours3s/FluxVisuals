package ru.fluxvisuals.util.render.capture;
...
import ru.fluxvisuals.mixin.EntityRenderDispatcherAccessor;
...
public final class EntityFramebufferCaptureManager {
   ...
   private boolean ensureCaptureFramebuffer(int width, int height) {
      ...
      if (framebuffer == null) {
         try {
            framebuffer = new SimpleFramebuffer("fluxvisuals_entity_capture", width, height, true);
   ...
   private void resetFrameResources() {
    ...
    private static boolean hasColorAttachment(Framebuffer framebuffer) {
       if (framebuffer == null) {
          ...
          return ru.fluxvisuals.util.render.utils.FramebufferUtils.getColorTextureId(framebuffer) > 0;
       }
    }
    ...
    private static int extractColorTextureId(Framebuffer framebuffer) {
       if (framebuffer == null) {
          ...
          return ru.fluxvisuals.util.render.utils.FramebufferUtils.getColorTextureId(framebuffer);
       }
    }
