...
import ru.fluxvisuals.util.render.utils.Color;
...
public final class WorldGeometryEmitter {
   ...
   public WorldGeometryEmitter(WorldRenderer renderer, Entry entry, VertexConsumer consumer) {
      this(Objects.requireNonNull(renderer, "renderer").camera(), entry, consumer, 2.0F);
   }

   public WorldGeometryEmitter(WorldRenderer renderer, Entry entry, VertexConsumer consumer, float lineWidth) {
      this(Objects.requireNonNull(renderer, "renderer").camera(), entry, consumer, lineWidth);
   }

   public WorldGeometryEmitter(Camera camera, Entry entry, VertexConsumer consumer) {
      this(camera, entry, consumer, 2.0F);
   }

   public WorldGeometryEmitter(Camera camera, Entry entry, VertexConsumer consumer, float lineWidth) {
   ...
   private void writeLineVertex(Vec3d worldPos, int rgbaColor, Vector3f normal) {
      ...
      vertex.lineWidth(this.lineWidth);
      this.finalizeVertex(vertex);
   }
