...
public final class Renderer2D {
   ...
   public void blur(float x, float y, float w, float h, float rTL, float rTR, float rBR, float rBL, float alpha) {
      this.ensureFrame();
      if (this.blurPrepared) {
         float opacity = clamp01(alpha) * this.currentAlphaMultiplier();
         if (!(opacity <= 1.0E-4F)) {
            this.backend.drawPreparedBlurRounded(x, y, w, h, Math.max(0.0F, rTL), Math.max(0.0F, rTR),
                  Math.max(0.0F, rBR), Math.max(0.0F, rBL), opacity, this.transformStack.current());
         }
      }
   }
   ...
   public static void setupOrientationMatrix(MatrixStack matrix, double x, double y, double z) {
      Camera camera = getMc().getEntityRenderDispatcher().camera;
