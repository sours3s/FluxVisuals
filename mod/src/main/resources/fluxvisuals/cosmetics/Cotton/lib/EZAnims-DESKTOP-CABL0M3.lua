...
local function setAnimation(anim,override,state,o)
    ...
    for _,value in pairs(saved.list) do
        if value:getName() == state..anim then
            local shouldPlay = saved.active and not override
            if shouldPlay then
                local wasStopped = value:isStopped()
                value:setPlaying(true)
                if wasStopped then
                    o.resumeTimes = o.resumeTimes or {}
                    local resume = o.resumeTimes[value]
                    if resume and (t - resume.tick) <= 5 then
                        value:setTime(resume.time)
                    end
                    o.resumeTimes[value] = nil
                end
            else
                if not value:isStopped() then
                    o.resumeTimes = o.resumeTimes or {}
                    o.resumeTimes[value] = {time = value:getTime(), tick = t}
                end
                value:stop()
            end
            exists = false
...
local function getInfo()
    ...
    local speed = velocity.xz:length()
    -- Hysteresis plus a short grace period to avoid idle/walk flicker near zero speed.
    if speed > 0.03 then
        movingState = true
        movingHold = 4
    elseif speed > 0.01 then
        movingState = true
    elseif movingHold > 0 then
        movingHold = movingHold - 1
        movingState = true
    else
        movingState = false
    end
    local moving = movingState
    ...
    local pv = velocity:mul(1, 0, 1)
    local pvLen = pv:length()
    if pvLen > 0 then
        pv = pv / pvLen
    else
        pv = vec(0, 0, 0)
    end
    local pl = models:partToWorldMatrix():applyDir(0,0,-1):mul(1, 0, 1)
    local plLen = pl:length()
    if plLen > 0 then
        pl = pl / plLen
    else
        pl = vec(0, 0, 0)
