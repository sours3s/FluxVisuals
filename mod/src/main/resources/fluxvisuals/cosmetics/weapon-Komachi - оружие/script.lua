...
function events.tick() 
    if pose then
        animations.komachi.fly:setPlaying(not pose)
    end

    if player:getItem(5).id ~= "minecraft:elytra" then
        models.komachi.Body.Elytra:setVisible(false)
    else
        models.komachi.Body.Elytra:setVisible(true)
    end
end
...
function events.render(delta, context)
    if context:find("FIRST") then
        ...
        if player:isLeftHanded() and not holdingSword then
            models.komachi.ItemScythe:setPos(-10,-10,-10)
        elseif player:isLeftHanded() or not holdingSword then
            models.komachi.ItemScythe:setPos(5,-10,-10)
        else
            models.komachi.ItemScythe:setPos(10,-10,-10)
        end
    else
        models.komachi.ItemScythe:setPos(0,0,0)
    end
end
...
function events.item_render(item)
    if item.id:find("sword") then
        return models.komachi.ItemScythe --Change this if you want
    end
end
...
function pings.flytoggle(state)
    pose = not pose
    animations.komachi.pose:setPlaying(state)
end
...
function pings.itemtoggle(state)
    models.komachi.ItemScythe:setVisible(not state)
end
