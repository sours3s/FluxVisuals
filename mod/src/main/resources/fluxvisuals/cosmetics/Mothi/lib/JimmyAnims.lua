...
local function compareVersionSafe(current, required)
  local ok, cmp = pcall(client.compareVersions, current, required)
  ...
  local function parseVersionNumbers(version)
    ...
    for part in tostring(version):gmatch("%d+") do
      numbers[#numbers + 1] = tonumber(part) or 0
    end
    return numbers
  end

  local left = parseVersionNumbers(current)
  local right = parseVersionNumbers(required)
...
assert(
  compareVersionSafe(client.getFiguraVersion(), avatarVer) > -1,
