-- config --
local config = {
    leftEar = models.MODELNAME.Main.upper.Neck.Head.EARS.EarRight, -- model part of left ear
    rightEar = models.MODELNAME.Main.upper.Neck.Head.EARS.EarLeft, -- model part of right ear

    velocityStrength = 1,

    extraAngle = 4, -- rotates ears by this angle when crouching
...
function events.tick()
    ...
    rot = rot + vel
    rot.x = rot.x + math.clamp(playerVel.z + playerRotVel.x, -4, 4)
    rot.z = rot.z + math.clamp(-playerVel.x, -6, 6)
    rot.w = rot.w + math.clamp(playerVel.y * 0.25, -4, 1)
end
