...
void main() {
    ...
    vec3 p = rayW * (uScale * 0.3 + 0.5);

    vec3 q = vec3(0.0);
    q.x = fbm(p + vec3(0.0, time * 0.4, 0.0));
    q.y = fbm(p + vec3(1.3, 0.8, time * 0.3));
    q.z = fbm(p + vec3(2.8, time * 0.5, 1.1));

    vec3 r = vec3(0.0);
    r.x = fbm(p + 1.2 * q + vec3(1.7, 9.2, time * 0.6));
    r.y = fbm(p + 1.2 * q + vec3(8.3, 2.8, time * 0.4));
    r.z = fbm(p + 1.2 * q + vec3(3.2, 1.4, time * 0.5));

    float f = fbm(p + r * 1.8);
    vec3 color1 = uColor;
    vec3 color2 = vec3(uColor.b, uColor.r, uColor.g);
    vec3 color3 = vec3(1.0) - uColor * 0.5;
    ...
    finalColor *= (f * 1.8 + uIntensity * 12.0);
    finalColor = mix(vec3(0.003, 0.003, 0.008), finalColor, uAlpha);
